...
void printBox(Box *box) {
    ...
    std::cout << "Box id: #" << std::internal << std::setfill('0') << std::setw(5) << std::hex << box->id << " " << std::dec;
    std::cout << "Width: " << box->width
              << ", Height: " << box->height << " at (" << box->x << ","
              << box->y << ")" << std::endl;
}
...
int main() {
  ...
  std::unique_ptr<PaddingBox> padding9c215 =
      std::make_unique<PaddingBox>(container7060f.get(), 4.0, 6.0, 5.0, 7.0, 0x9c215);

  std::unique_ptr<PaddingBox> padding3b44a =
      std::make_unique<PaddingBox>(padding9c215.get(), 8.0, 8.0, 8.0, 8.0, 0x3b44a);

  std::unique_ptr<PaddingBox> padding154a4 =
      std::make_unique<PaddingBox>(padding3b44a.get(), 16.0, 16.0, 16.0, 16.0, 0x154a4);
